...
local BedwarsControllers = {
    matchState = ClientStoreHandle:getState().Game.matchState,
...
function FunnyKillAuraRemote()
    for i,v in pairs(game.Players:GetChildren()) do
        if v ~= game.Players.LocalPlayer and isAlive(v) and isAlive() then
            if (v.Character.HumanoidRootPart.Position - lplr.Character.HumanoidRootPart.Position).Magnitude < DistanceValue and v.Team ~= game.Players.LocalPlayer.Team then
...
function DestroyNearestBeds()
    for i, v in pairs(game:GetService("Workspace").Map.Worlds[MapName].Blocks:GetChildren()) do
        if isAlive() and v.Name == "bed" and v.Covers.BrickColor ~= game.Players.LocalPlayer.Team.TeamColor and isAlive() then
            if (v.Position - game.Players.LocalPlayer.Character.PrimaryPart.Position).Magnitude < 27 then
...
-- KillAura
local k_funny = c_killaura:addToggle("Funny Aura")
...
local origC02 = game.Players.LocalPlayer.Character.RightHand.RightWrist.C0
...
function LJump(LSpeed)
    local c = dnec(l_humrp.Changed)
    local d = dnec(l_humrp:GetPropertyChangedSignal("CFrame"))
    moveDir = lplr.Character.Humanoid.MoveDirection ~= Vector3.zero and lplr.Character.Humanoid.MoveDirection or l_humrp.CFrame.lookVector
    velo = moveDir * (LSpeed - l_plr.Character.Humanoid.WalkSpeed) * skipFrame()
...
p_speed:Connect("Toggled", function(t)
    if t and not FlyOn then
        ...
        servRun:BindToRenderStep("Speed", 2000, function(dt)
            if isAlive() then
                lplr.Character.HumanoidRootPart.CFrame += lplr.Character.Humanoid.MoveDirection * dt * 5 * SpeedVal -- fake error lmao
            end
        end)
    else 
...
p_nofall:Connect("Toggled", function(t)
    if (t) then
        servRun:BindToRenderStep("NoFall", 2000, function(dt)
            if (t) then
        ...
        servRun:UnbindFromRenderStep("NoFall")
    end
end)
